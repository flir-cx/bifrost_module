...
#ifdef __SIM
static int map_vmalloc_area(struct vm_area_struct *vma)
{
    unsigned long start = vma->vm_start;
    unsigned long length = vma->vm_end - vma->vm_start;
    unsigned long vmalloc_area_ptr = vma->vm_pgoff << PAGE_SHIFT;
    unsigned long pfn;
...
/* Need at least 22 byte to convert the address into strings */
static void *get_phys_mem(char **start, char **end, char *buf)
{
        void *p;
        size_t n;
        ...
        *start = buf;
        buf += sprintf(*start, "0x%lx", (unsigned long)p);
        
        *end = buf + 1;
        buf += sprintf(*end, "0x%lx", (unsigned long)p + n - 1);

        printk("CMEMK allocated %u MBYTE, using vmalloc, for "
               "pools and heap\n", (unsigned)n / (1 MBYTE));

        return p;
}

static void put_phys_mem(void *p)
{
        printk("CMEMK freed memory block\n");
        vfree(p);
}
#endif
...
#ifdef __SIM
    /*
     * Let's give the CMEM allocator some memory. This memory will
     * be used for pools and heap. The heap gets what's left after
     * the pools has been allocated.
     *
     * CMEM can use upto two memory banks, but we'll start with
     * just one.
     *
     * Memory bank parmeters (strings)
     * phys_start => "0x..."
     * phys_end   => "0x..."
     * pool_table => "<num-bufs>x<pool-size>" (decimal values)
     */
    physmem = get_phys_mem(&phys_start, &phys_end, tmpbuf);
    if (physmem == NULL) {
            __E("couldn't allocate physical memory\n");
            err = -ENOMEM;
...
/* cut-and-paste above as part of adding support for more than 2 blocks */
	...
	/*
	 * This overlap check doesn't work if more than one bank of
	 * physical memory is passed to the kernel. Can't use num_physpages
	 * to calculate the end of the physical memory (it only tells
        ...
        if (phys_end_kernel > block_start[bi]) {
            if (allowOverlap == 0) {
                __E("CMEM phys_start (%#lx) overlaps kernel (%#lx -> %#lx)\n",
                    block_start[bi], (long)virt_to_phys((void *)PAGE_OFFSET), (long)phys_end_kernel);
                err = -EINVAL;
                goto fail_after_create;
            }
            else {
                ...
                printk("CMEM phys_start (%#lx) overlaps kernel (%#lx -> %#lx)\n",
                   block_start[bi], (long)virt_to_phys((void *)PAGE_OFFSET), phys_end_kernel);
            }
        }
#endif
