...
{
    uint32_t      BASE_ADDRESS              =  0x0000E00;


    /* Masks used with register CONTROL1, CONTROL2, CONTROL3 */
    uint32_t      BIT_MASK_10_0             = (0x7FF << 0);   ///< bit[10:0]
    uint_fast8_t  BIT_SHIFT_0               =  0;
    uint32_t      BIT_MASK_26_16            = (0x7FF << 16);  ///< bit[26:16]
    uint_fast8_t  BIT_SHIFT_16              =  16;
    ...
    uint32_t      BORDER_COLOR_B_MASK       = (0x3FF << 0);   ///< bit[9:0] = Border color B
    uint_fast8_t  BORDER_COLOR_B_SHIFT      =  0;
    uint32_t      BORDER_COLOR_G_MASK       = (0x3FF << 10);  ///< bit[19:10] = Border color G
    uint_fast8_t  BORDER_COLOR_G_SHIFT      =  10;
    uint32_t      BORDER_COLOR_R_MASK       = (0x3FF << 20);  ///< bit[29:20] = Border color R
    uint_fast8_t  BORDER_COLOR_R_SHIFT      =  20;

    /* CONTROL5 register */
    uint32_t      CONTROL5_OFFSET           =  0x0020;        ///< offset relative to unit base address
    uint32_t      DEBUG_CLEAR_MASK          =  (1 << 0);      ///< bit[0] = Debug clear

    /* STATUS1 register */
    uint32_t      STATUS1_OFFSET            =  0x0080;        ///< offset relative to unit base address
    uint32_t      WRITE_WHEN_FIFO_FULL_MASK = (1 << 0);       ///< bit[0] = Write when FIFO full
    uint32_t      READ_WHEN_FIFO_EMPTY_MASK = (1 << 1);       ///< bit[1] = Read when FIFO empty
