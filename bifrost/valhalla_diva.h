...
{
    const uint32_t      BASE_ADDRESS              =  0x00001E00;

    /* CONTROL1 register */
    const uint32_t      CONTROL1_OFFSET           =  0x0010;      ///< offset relative to unit base address
    const uint32_t      DIVA_ENABLE_MASK          = (1 << 0);     ///< bit[0] = DIVA enable
    const uint32_t      CLEAR_DEBUG_STATUS_MASK   = (1 << 31);    ///< bit[31] = Clear debug status
    ...
    /* CONTROL6 register */
    const uint32_t      CONTROL6_OFFSET           =  0x0024;      ///< offset relative to unit base address
    // bit[15:0]  = Sample no start for DIVA_DE
    // bit[31:16] = Line no start for DIVA_DE

    /* CONTROL7 register */
    const uint32_t      CONTROL7_OFFSET           =  0x0028;      ///< offset relative to unit base address
    // bit[15:0]  = Sample no stop for DIVA_DE
    // bit[31:16] = Line no stop for DIVA_DE

    const uint32_t      HIGH_2_BYTE_MASK          = (0xFF << 16); ///< bit[31:16]
    const uint_fast8_t  HIGH_2_BYTE_SHIFT         =  16;
    const uint32_t      LOW_2_BYTE_MASK           = (0xFF << 0);  ///< bit[15:0]
    const uint_fast8_t  LOW_2_BYTE_SHIFT          =  0;

    /* STATUS1 register */
    const uint32_t      STATUS1_OFFSET            =  0x0080;      ///< offset relative to unit base address
    const uint32_t      DIVA_ID_MASK              = (0xF << 0);   ///< bit[3:0] = DIVA id
    const uint_fast8_t  DIVA_ID_SHIFT             =  0;
    const uint32_t      READ_WHEN_FIFO_EMPTY_MASK = (1 << 30);    ///< bit[30] = Read when FIFO empty
    const uint32_t      WRITE_WHEN_FIFO_FULL_MASK = (1 << 31);    ///< bit[31] = Write when FIFO full
}
